...
public class MNSQueueConsumerBean extends MNSQueueConsumer implements InitializingBean, ApplicationContextAware {
    ...
    private void setMnsClient() throws Exception {
        ...
        if (bean == null || !(bean instanceof MNSClient)) {
            logger.error("unexpected bean[{}], not MNSClient",
    ...
    private void setMessageProcessor() throws Exception {
        ...
        if (bean == null || !(bean instanceof MessageProcessor)) {
            logger.error("unexpected bean[{}], not MessageProcessor",
                    bean == null ? "null" : bean.getClass().getName());
            throw new Exception(String.format("unexpected bean[%s]",
                    bean == null ? "null" : bean.getClass().getName()));
        }
